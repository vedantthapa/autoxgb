...
def train_model(model_config):
    ...
    if model_config.baseline:
        best_params = xgb_model().get_xgb_params()
        ...
        study.optimize(optimize_func, n_trials=model_config.num_trials,
...
def predict_model(model_config, best_params):
    ...
    for fold in range(model_config.num_folds):
        ...
        if model_config.test_file is not None:
            test_feather = pd.read_feather(os.path.join(
                model_config.output, f"test_fold_{fold}.feather"))
    ...
    mean_metrics = dict_mean(scores)
    logger.info(f"Metrics: {mean_metrics}")
    save_valid_predictions(final_valid_predictions,
                           model_config, target_encoder, "oof_predictions.csv")

    if model_config.test_file is not None:
        save_test_predictions(final_test_predictions, model_config,
                              target_encoder, test_ids, "test_predictions.csv")
    else:
        logger.info(
            "No test data supplied. Only OOF predictions were generated")
